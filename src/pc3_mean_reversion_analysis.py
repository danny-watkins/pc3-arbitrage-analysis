...
Date: 2025
Description:
Analyzes the mean-reverting behavior of PC3 dislocations across different market regimes.
Market regimes include low volatility, high volatility, and monetary policy change periods.
Generates visualizations for PC3 dislocations, half-life analysis, correlation with VIX,
and distribution of dislocations. Includes validation using the Augmented Dickey-Fuller test.
"""
...
# Panel 1: Time Series of PC3 Dislocations with Market Regime Shading
plt.subplot(2, 2, 1)

# Plot PC3 Dislocations
sns.lineplot(data=merged_data, x=merged_data.index, y='PC3', label="PC3 Dislocations", color="blue", linewidth=1.5)
plt.axhline(y=0, color='black', linestyle='--', linewidth=1, label="Mean Reversion Line")

# Shade regions for market regimes (background shading)
for regime, color in zip(['Low Volatility', 'High Volatility', 'Monetary Policy Change'], ['green', 'red', 'orange']):
    ...
    if not regime_dates.empty:
        for i in range(len(regime_dates) - 1):
            if regime_dates[i + 1] - regime_dates[i] == pd.Timedelta(days=1):  # Ensure consecutive dates
                plt.axvspan(regime_dates[i], regime_dates[i + 1], color=color, alpha=0.2, label=regime if i == 0 else "")
...
# Add annotations for market regimes
plt.text(pd.to_datetime('2014-01-01'), 1.5, "Low Volatility", color='green', fontsize=12, alpha=0.8)
plt.text(pd.to_datetime('2020-03-01'), 1.5, "High Volatility", color='red', fontsize=12, alpha=0.8)
plt.text(pd.to_datetime('2019-08-01'), -1.5, "Policy Change", color='orange', fontsize=12, alpha=0.8)

plt.title("PC3 Dislocations with Market Regime Shading")
plt.xlabel("Date")
plt.ylabel("PC3 Dislocation")
plt.legend(loc='upper left')

# Panel 2: Half-Life Bar Chart
plt.subplot(2, 2, 2)
...
# Panel 4: Distribution of PC3 Dislocations
plt.subplot(2, 2, 4)
...
plt.tight_layout()
plt.savefig("visuals/multi_panel_plot.png")
plt.show()
