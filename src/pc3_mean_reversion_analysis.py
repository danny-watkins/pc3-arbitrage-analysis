...
Updated: 2025
Description:
Analyzes mean-reversion properties of raw PC1, PC2, PC3 scores
...
# -------------------- Load Data --------------------
raw_pcs = pd.read_csv(os.path.join(data_dir, "rolling_pca_results.csv"), parse_dates=["date"])
raw_pcs.set_index("date", inplace=True)
vix_data = pd.read_csv(os.path.join(data_dir, "vix_data.csv"), parse_dates=["Date"])
vix_data.set_index("Date", inplace=True)

pc_scores = raw_pcs[["PC1_Score", "PC2_Score", "PC3_Score"]]
...
print("\n[INFO] Mean Reversion Analysis (Raw PC Scores):\n")
for pc, metrics in analysis_results.items():
    ...
    print("  [INFO] Stationary (mean-reverting)" if metrics['ADF_pvalue'] < 0.05 else "  [INFO] Not stationary")
    print("")
...
plt.plot(pc_scores.index, pc_scores["PC1_Score"], label="PC1 (Level)", color="green", alpha=0.7)
plt.plot(pc_scores.index, pc_scores["PC2_Score"], label="PC2 (Slope)", color="orange", alpha=0.7)
plt.plot(pc_scores.index, pc_scores["PC3_Score"], label="PC3 (Curvature)", color="blue", linewidth=2)
plt.plot(vix_norm.index, vix_norm, label="VIX (Z-Score)", color="gray", linestyle="--", alpha=0.5)
plt.title("Raw PC Scores Over Time with VIX")
plt.xlabel("Date")
plt.ylabel("Score Value")
plt.grid(True)
plt.legend()
plt.tight_layout()
plt.savefig(os.path.join(visuals_dir, "raw_pc_scores_with_vix.png"))
plt.close()
print(f"[INFO] Saved plot of raw PC scores with VIX overlay to '{visuals_dir}/raw_pc_scores_with_vix.png'")

# -------------------- Heatmap: PCs vs VIX --------------------
merged = pc_scores.join(vix_data["VIX"], how="inner")
corr_matrix = merged.corr().loc[
    ["PC1_Score", "PC2_Score", "PC3_Score"], ["VIX"]
]

plt.figure(figsize=(6, 4))
sns.heatmap(corr_matrix, annot=True, cmap="coolwarm", center=0)
plt.title("Correlation of Raw PCs with VIX")
plt.tight_layout()
plt.savefig(os.path.join(visuals_dir, "raw_pc_vix_correlation_heatmap.png"))
plt.close()
print(f"[INFO] Saved PC–VIX correlation heatmap to '{visuals_dir}/raw_pc_vix_correlation_heatmap.png'")
